...
public class Eleven {
	...
	public static void main(String args[]) {
		...
		String[] gridSplit = grid.toString().split(" ");

		System.out.println("grdiSplit size " + gridSplit.length);

		int x = -1;
		for (int y = 0; y < gridSplit.length; y++) {
			if (y % DIM == 0) {
				x++;
			}
			System.out.println("y " + y % DIM + "x " + x);
			matrix[x][y % DIM] = Integer.parseInt(gridSplit[y]);
		}
		...
		for (int i = 0; i < DIM; i++) {
			for (int j = 0; j < DIM; j++) {
				...
				if (tempProd > prod) {
					prod = tempProd;
					System.out.println("Horisontal " + tempProd);
				}
				...
				if (tempProd > prod) {
					prod = tempProd;
					System.out.println("Vertical " + tempProd);
				}
				...
				if (tempProd > prod) {
					prod = tempProd;
					System.out.println("Diagonal a " + tempProd);
				}
				...
				if (tempProd > prod) {
					prod = tempProd;
					System.out.println("Diagonal b " + tempProd);
				}
			}
		}
